...
def kmeans(dataSet,k):
    ...
    #1st column stores which cluster this sample belongs to
    #2ed column stores the error between sample and its centroid
...
def biKmeans(dataSet,k):
    numSamples=dataSet.shape[0]
    clusterAssment=mat(zeros((numSamples,2)))
    #step1:init cluster is the whole data set
    ...
    while len(cenList)<k:
        minSSE=100000.0
        numCurrCluster=len(cenList)
        for i in range(numCurrCluster):
            #step2:get samples in cluster i
            pointsInCurrCluster = dataSet[nonzero(clusterAssment[:,0].A==i)[0],:]
            #step3:cluster it to 2 sub-clusters using k-means
            centroids,splitClusterAssment=kmeans(pointsInCurrCluster,2)
            #step4:calculate the sum of square error after split this cluster
            ...
            #step5:find the best split cluster which has the min sum of square error
            if currSplitSSE<minSSE:
                minSSE =currSplitSSE
                bestCentroidToSplit =i
                bestNewCentroids=centroids.copy()
                bestClusterAssment=splitClusterAssment.copy()
        #step6:modify the cluster index for adding new cluster
        ...
        #step7:update and append the centroids of the new 2 sub-cluster
        cenList[bestCentroidToSplit]=bestNewCentroids[0,:]
        cenList.append(bestNewCentroids[1,:])

        #step8:update the index and error of the samples whose cluster have been changed
